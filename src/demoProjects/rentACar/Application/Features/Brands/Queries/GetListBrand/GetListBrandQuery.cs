...
namespace Application.Features.Brands.Queries.GetListBrand
{
    public class GetListBrandQuery : IRequest<BrandListModel>
    {
        ...
        public class GetListBrandQueryHandler : IRequestHandler<GetListBrandQuery, BrandListModel>
        {
            ...
            {
                IPaginate<Brand> brands = await _brandRepository.GetListAsync(index: request.PageRequest.Page, size: request.PageRequest.PageSize); // Veri tabanından veriler gelir
                
                BrandListModel mappedBrandListModel = _mapper.Map<BrandListModel>(brands); // Dbden aldıklarını modeldaki Page kısmına atıyorum
